...
r_cm = 72.1 * au          # радиальное расстояние от звезды, а.е.
...
beta = 18.0                # плазменный параметр в диске
...
rho0 = 1.07e-13   # температура в экватор. плоскости диска, г/см^3
T0 = 10       # температура внутри диска, К
c_Td = np.sqrt(Rg * T0 / mu) # скорость звука внутри диска, см/с
v0 = c_Td
H = 3.68*au     # шкала высот диска, см
...
Ta = 100                    # температура в атомосфере, К
...
def SaveData(n, t_n):
    ...
    np.savetxt("./out/data" + str(n) + ".dat", np.array(data).transpose(), fmt=('%.3e', '%.3e', '%.3e', '%.3e', '%.3e', '%.3e'), header = ("%.2e" % t_n), comments="")
...
summary_file = open("./out/model_summary.txt", "w")
